...
// MODIFIED: Tim Bourque 8 Nov 2018
// MODIFIED: Tim Bourque 10 Nov 2018
...
{
	double distance = 0, numOf;
	for (int i = 1; i < number.size()-1; i++) {
		if (abs(number.at(i) - number.at(i - 1)) > distance) {
			distance = abs(number.at(i) - number.at(i - 1));
			numOf = number.at(i);
		}
		if (abs(number.at(i + 1) - number.at(i)) > distance) {
			distance = abs(number.at(i + 1) - number.at(i));
			numOf = number.at(i);
		}
	}
	return numOf;
